...
var NepaliView = View.extend({
	...
	numberCellHtml: function(cell) {
		var date = cell.start;
		var classes;
		var formatted = date.format();
		var nepaliDateObj = NepaliDateConversion.toNepali(formatted);
		var nepaliYear = NepaliDates.getNumber(nepaliDateObj.year);
		var nepaliMonth = NepaliDates.getMonth(nepaliDateObj.month);
		var nepaliDay = NepaliDates.getNumber(nepaliDateObj.day);
		var nepaliFormatted = nepaliYear + '-' + nepaliMonth + '-' + nepaliDay;
		...
		return '' +
			'<td class="' + classes.join(' ') + '" data-date="' + formatted + '" data-date-nepali="' + nepaliFormatted + '">' +
				// date.date() +
				'<small class="fc-nepali-month">' + nepaliMonth + '</small>' +
				nepaliDay +
			'</td>';
	},
